...
namespace Library.Utils
{
    ...
    {
        internal Logger(string fileName, string uuid, int day, string time, string message) {
            List<RLogs> logs = new();

            FileClass file = new();
            List<RPeople> data = file.Get<RPeople>(fileName);
            logs.Union(data.First(person => person.UUID == uuid).Logs);
            ...
            file.Save(JArray.Parse(data.ToString()), fileName);
        }
